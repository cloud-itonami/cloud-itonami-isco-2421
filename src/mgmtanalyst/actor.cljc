(ns mgmtanalyst.actor
  "ManagementOrganizationAnalystsActor — the ISCO-08 2421 community mgmtanalyst actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one mgmtanalyst operation request
  ...
  The unconditional invariant: the ManagementOrganizationAnalystsAdvisor can never
  directly commit a record the ManagementOrganizationAnalystsGovernor refuses —
...
(defn build-graph
  ...
  `mgmtanalyst.store/Store`. `advisor` implements
  `mgmtanalyst.advisor/Advisor` (defaults to `mock-advisor`).
