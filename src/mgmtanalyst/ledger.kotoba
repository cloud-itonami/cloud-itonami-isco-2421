(ns mgmtanalyst.ledger
  "Append-only audit ledger for the ISCO-08 2421 management and organization
  analysts actor: entry construction, hash chaining, and verification.
  ...
  audit is trying to check. For a record of which recommendations were
  approved and which savings claims were made to a client, that is the
  difference between an audit that can show what was advised and one that can
  ...
  a `:publish-recommendation` committed through human sign-off and a routine
  `:approve-recommendation` committed automatically both produced
...
(defn summary
  "One line per entry, for operators and for `mgmtanalyst.sim` output."
  [ledger]
  (apply str
         (interpose "\n"
                    (for [e ledger]
